...
public:
  ...
  DES(const std::bitset<64>& key){
    // Aquí se podría implementar la generación de claves
    // a partir de la clave inicial de 64 bits.
  ...
  std::bitset<64> iPermutation(const std::bitset<64>& input){
    ...
    // Implementación de la permutación inicial (IP)
    // Aquí se usaría una tabla de permutación real.
    for (int i = 0; i < 64; ++i) {
      output[i] = input[i]; // Simplificación, usar tabla real si se desea
    }
    return output;
  }
